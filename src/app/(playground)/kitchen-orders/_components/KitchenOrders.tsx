...
function KitchenOrders() {
  ...
  return (
    <>
      <Button className="btn mb-5" onClick={() => router.push("/kitchen")}>
        Vissza a kijelzőhöz
      </Button>
              ...
              {data?.items.map((order, idx) => {
                return (
                  <TableRow className="text-lg" key={idx}>
                    ...
                    {order.finishedCokingTime && !order.finishedTime && (
                      <TableCell>
                        <Button
                          onClick={() =>
...
const handleOnClick = async (token: null | string, orderId: string) => {
  ...
  try {
    const resp = await fetch(`${ENDPOINTURL}/order/revert/finish/${orderId}`, {
      method: "PATCH",
      headers: { Authorization: token },
    });
    if (!resp.ok) {
