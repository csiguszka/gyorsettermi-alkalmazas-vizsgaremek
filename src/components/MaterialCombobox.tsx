...
export function MaterialCombobox({ onMaterialSelect }: { onMaterialSelect: (material: Material) => void }) {
...
async function getMaterials(token: string | null): Promise<Material[]> {
  ...
  try {
      ...
      throw new Error(error instanceof Error ? `Hiba történt: ${error.message}` : "Ismeretlen hiba történt");
  }
}
